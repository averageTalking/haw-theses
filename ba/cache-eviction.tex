\subsection{Cache Eviction} \label{subsec:cache-eviction}
    ...
    attacks represent a powerful method for exploiting the memory hierarchy of modern system architectures~\cite{thesisCacheAttacksandRH}. 
    To evict a specific address from the cache back to main memory, attackers can access congruent 
    ...
    strategy~\cite{thesisCacheAttacksandRH}. Over time, various methods have been proposed to reliably evict data from the cache.
    
    \subsubsection{Cache Attacks} \label{subsubsec:cache-attacks}
        % Flush+Reload
        The \textbf{\textit{Flush+Reload}} attack was introduced by Gullasch et al.~\cite{paperCacheGames} and later extended by
        Yarom and Falkner~\cite{paperFlushReload}. It exploits shared memory between an attacker and a victim and provides very 
        fine-grained resolution~\cite{thesisMicroAttacks, paperRowhammerjs}. The basic idea proposed in~\cite{paperCacheGames} is 
        ...
        instructions. Yarom and Falkner~\cite{paperFlushReload} further observed that the \texttt{clflush} instruction evicts cache 
        lines from all cache levels, including the shared \gls{llc}, which enables \textit{Flush+Reload} 
        attacks even when the attacker and victim execute on different \gls{cpu} cores~\cite{thesisCacheAttacksandRH}.
        \\
        ...
        using \textit{Flush+Reload}~\cite{paperCacheTemplateAttacks}. Additionally, Yarom et al. successfully recovered OpenSSL ECDSA nonces and, 
        consequently, the private key using this technique~\cite{paperECDSA}.
        \\
        ...
        like \texttt{mmap}, as seen in \cref{fig:flush-and-reload}a. Next, a specific cache line associated 
        with this shared memory region is evicted from the entire cache hierarchy, including the \gls{llc}, using a cache 
        flush instruction (see \cref{fig:flush-and-reload}b). After the victim process is 
        scheduled and executes (see \cref{fig:flush-and-reload}c), the attacker re-accesses the previously 
        flushed memory location. The access latency is then measured to distinguish between a cache hit and a cache 
        miss, as seen in \cref{fig:flush-and-reload}d. A low access time indicates that the data was reloaded 
        ...
        \begin{figure}[htbp]
            ...
            \caption{\textit{Flush+Reload} - 4-way (columns) cache with 8 sets (rows)~\cite{thesisCacheAttacksandRH}}
            \label{fig:flush-and-reload}
        \end{figure}
        As seen in~\cref{fig:result_flushandreload}, the experimental results from \textit{Flush+Reload} measurements on the Raspberry Pi 5 reveal three distinct peaks in 
        ...
        detectable via hardware performance counters~\cite{thesisCacheAttacksandRH}. The \textbf{\textit{Flush+Flush}} attack addresses this limitation 
        by relying on the execution time of the flush instruction, which differs depending on whether the 
        targeted data is cached or not~\cite{paperFlushFlush}. The attack setup is similar to \textit{Flush+Reload}. A shared or binary object 
        file is mapped into the attacker's address space, an address is flushed from the cache, and the victim program 
        is scheduled. Yet, after the victim executes, the attacker flushes the address again instead of reloading 
        it. This second flush does not generate cache misses, unlike \textit{Flush+Reload} or \textit{Prime+Probe}.
        \\
        Gruss et al.~\cite{paperFlushFlush} demonstrated the effectiveness of \textit{Flush+Flush} by constructing a covert channel with a 
        ...
        minimum due to the continuous cache flushes as seen in~\cite{paperFlushFlush} or on all of our test devices~\cite{thesisMicroAttacks}.
        \\ \\ % Evict+Time
        The \textbf{\textit{Evict+Time}} attack measures how evicting a specific cache set affects the execution time~\cite{thesisCacheAttacksandRH}. 
        ...
        \\
        The results can be taken from~\cref{fig:result_evictandtime}.
        On the one hand, the Raspberry Pi 4, experimental results show a clear separation between cache-resident execution at 
        ...
        of about 259 \gls{cpu} cycles. On the other hand, the Raspberry Pi 5's cache and memory access times nearly coincide at roughly 
        ...
        identify which cache sets are used during a victim’s computations~\cite{thesisCacheAttacksandRH}. The attack operates in a manner similar 
        ...
        time. Due to the pseudo-random cache replacement policy~\cite{thesisCacheAttacksandRH}, it is possible that accesses during the probing phase evict 
        previously accessed attacker addresses, which can lead to false positives.
        \\
        \textit{Prime+Probe} has been used in several attacks. In 2015, Irazoqui et al. applied \textit{Prime+Probe} to attack the OpenSSL 
        AES implementation in a cloud environment~\cite{paperSharedCacheAttack}. In the same year, Liu et al.~\cite{paperLastLevelCacheAttack} used \textit{Prime+Probe} to construct a 
        cross-core covert channel and to attack ElGamal decryption in GnuPG~\cite{paperLastLevelCacheAttack}.
        \begin{figure}[htbp]
            \centering
            \frame{\includegraphics[width=\linewidth]{img/prime-and-probe.pdf}}
            \caption{\textit{Prime+Probe} - 4-way (columns) cache with 8 sets (rows)~\cite{thesisCacheAttacksandRH}}
            \label{fig:prime-and-probe}
        \end{figure}
        Experimental results, as seen in~\cref{fig:results_primeandprobe}, show that \textit{Prime+Probe} fails on the Raspberry Pi 4 when using the selected cache eviction 
        strategy. On the Raspberry Pi 5, observable responses exhibit unusual behavior. We observed instead of two distinct peaks 
        corresponding to cache hits and cache misses, a single broad peak spanning approximately 500 to 1000 \gls{cpu} cycles.
        ARM CPUs employ a pseudo-random cache replacement policy to determine which cache line within a 
        set is evicted, which introduces additional noise into time-driven cache attacks~\cite{paperTimeDrivenCacheAttacks}. This behavior has been 
        identified as a key factor preventing the successful application of \textit{Prime+Probe} attacks on ARM architectures 
        so far~\cite{paperCacheAccessPattern, thesisCacheAttacksandRH}.
        \begin{figure}[htbp]
            \centering
        ...
        flush instruction with cache eviction~\cite{paperCacheTemplateAttacks}. While it has no practical use on x86 CPUs due to the availability 
        of the unprivileged \texttt{clflush} instruction, it is applicable to ARM CPUs that lack an unprivileged 
        cache flush instruction~\cite{paperDrammer}. Cache eviction in \textit{Evict+Reload} requires filling a cache set with a sufficient 
        ...
        or accessing them insufficiently results in a low eviction rate~\cite{thesisCacheAttacksandRH}. Gruss et al. later demonstrated 
        that effective eviction strategies can even trigger \gls{dram} bitflips, known as Rowhammer, via JavaScript 
        without relying on the privileged \texttt{clflush} instruction~\cite{paperRowhammerjs}.
        \\
        The results from~\cref{fig:result_evictandreload} obtained with \textit{Evict+Reload} are consistent with those observed using \textit{Flush+Reload}. On the 
    ...
    \subsubsection{Feasibility Constraints} \label{subsubsec:feasibility-constraints}
        ...
        side-channel attacks or Rowhammer exploits theoretically feasible. In practice, the execution
        ...
        policies~\cite{thesisCacheAttacksandRH}. This increased noise exceeds that of cache flushing, thereby increasing the number of 
        measurements required~\cite{thesisDRAMA} and further reducing efficiency. Moreover, the most efficient and effective 
        cache eviction strategy must be determined empirically~\cite{toolArmageddon}, and even the most efficient known strategies 
        remain too slow to reliably trigger the Rowhammer bug on ARM architectures~\cite{webCache}.
