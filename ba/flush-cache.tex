\subsection{Flush Cache} \label{subsec:flush-cache}
    ...
    other~\cite{datasheetCortexASeries}. This operation is only applicable to data caches that use a write-back policy. Cache 
    ...
    data cache, and translation table walks of a core are guaranteed to observe the same copy of a memory location~\cite{datasheetCortexASeries}. 
    In a system with Harvard L1 caches and a \gls{tlb} for caching translation table entries, a unified L2 cache represents 
    the Point of Unification. If no external cache is present, the main memory itself serves as the \gls{pou}~\cite{thesisCacheAttacksandRH}, as 
