\section{Fast} \label{sec:fast}
    To achieve P3. Fast, memory accesses must race against the next row refresh in order to induce 
    ...
    \subsection{Target Row Refresh Detection Approaches} \label{subsec:trr-detection-approaches}
        ...
        part numbers of the \gls{dram} components were determined, as summarized in 
        \hyperref[tab:fbga-part-number-decoder]{Table~\ref{tab:fbga-part-number-decoder}}.
        ...
        \\
        An analysis of the relevant mode register configuration, as summarized in 
        \hyperref[tab:mode-register-assignments]{Table~\ref{tab:mode-register-assignments}}, 
        ...
        However, the exact device behavior is vendor-specific.
        ...
        contents according to the corresponding DDR standard.
        \\
    ...
    \subsection{Bypass Techniques} \label{subsec:bypass-techniques}
        ...
        natively rebuilding the Linux kernel, as described in \hyperref[sec:lab-setup]{Lab Setup}.
        \\
        ...
        \\
        After enabling HugePages, their availability was verified using 
        \texttt{ls /sys/kernel/mm/hugepages/}, which reports the supported HugePage sizes. On the 
        Raspberry Pi 4, the available sizes were 64 KB, 2 MB, 32 MB, and 1 GB, whereas on the 
        Raspberry Pi 5 only 2 MB, 32 MB, and 1 GB HugePages were supported. To ensure deterministic 
    ...
    \subsection{TRRespass Port} \label{subsec:trrespass-port}
        This is a test.
