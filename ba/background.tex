\section{Background} \label{sec:background}
  \subsection{CPU Caches} \label{subsec:-caches}
    ...
    relatively low cost [27]. In contrast, Static Random-Access Memory (\gls{sram}) maintains each bit 
    ...
    though it is more expensive and offers lower storage capacity compared to \gls{dram} [27]. \\
    \gls{cpu} caches are constructed from \gls{sram} to maximize access speed [45, 31]. A cache is a fast, 
    ...
    to access imminently [27, 45]. By holding data that the \gls{cpu} may repeatedly access, caches 
    ...
    storage devices such as \gls{ram} or disks [27]. Common types of caches include \gls{cpu} caches, memory 
    caches, and browser caches [27]; however, only \gls{cpu} caches are relevant in the context of the 
    Rowhammer attack. \\
    ...
    speeds approximately 10–100 times faster than \gls{dram} [46].