\subsection{Flush Cache} \label{subsec:flush-cache}
    ...
    attacks. On x86 architectures, this behavior is implemented via the \texttt{clflush} instruction~\cite{paperFlippingBits}, whereas 
    ARM-based systems rely on cache flush maintenance instructions, such as \texttt{DC CIVAC}, \texttt{DC CVAC}, \texttt{DC ZVA}, and 
    related operations, to achieve comparable effects~\cite{paperTriggeringRHHardwareFault}.
    \\
    In ARM terminology, the word flush is often used in descriptions of clean and invalidate 
    operations, although the architecture specification generally uses only the terms clean and invalidate~\cite{thesisCacheAttacksandRH}.
    \\
    \textit{\textbf{Invalidating}} a cache or cache line means clearing it of data by clearing the valid bit of one 
    ...
    write-back cacheable regions would be lost~\cite{datasheetCortexASeries}.
    \\
    \textit{\textbf{Cleaning}} a cache or cache line refers to writing the contents of dirty cache lines back to main memory 
    and clearing the dirty bits, which makes the contents of the cache and main memory coherent with each 
    other~\cite{datasheetCortexASeries, thesisCacheAttacksandRH}. This operation is only applicable to data caches that use a write-back policy. Cache 
    clean and invalidate operations can be performed by cache set or way, or by virtual address.
    \\
    The \textit{\textbf{\gls{poc}}} is defined as the point at which all blocks that can access memory, such 
    as processor cores or DMA engines, are guaranteed to observe the same copy of a given address~\cite{datasheetCortexASeries}. 
    Commonly, this point corresponds to the main external system memory, as illustrated in~\cref{fig:poc-pou}b. 
    \\
    Closely related is the \textit{\textbf{\gls{pou}}}, which denotes the point in the memory hierarchy at which the instruction cache, 
    data cache, and translation table walks of a core are guaranteed to observe the same copy of a memory location~\cite{datasheetCortexASeries, thesisCacheAttacksandRH}. 
    In a system with Harvard L1 caches and a \gls{tlb} for caching translation table entries, a unified L2 cache represents 
    the Point of Unification~\cite{datasheetCortexASeries}. If no external cache is present, the main memory itself serves as the \gls{pou}~\cite{thesisCacheAttacksandRH}, as 
    also shown in~\cref{fig:poc-pou}a.
    \begin{figure}[htbp]
        \centering
        \frame{\includegraphics[width=\linewidth]{img/poc-pou.pdf}}
        \caption{Point of Coherency and of Unification~\cite{datasheetCortexASeries}}
        \label{fig:poc-pou}
    \end{figure}

    \subsubsection{Instruction Types} \label{subsubsec:instruction-types}
        The original Rowhammer study by Kim et al.~\cite{paperFlippingBits} employed the \texttt{clflush} instruction to force cache lines 
        out of the cache hierarchy. \texttt{clflush} ensures that each memory access reaches physical \gls{dram}, thereby 
        enabling the high-frequency row activations required for Rowhammer. However, Raspberry Pi devices 
        ...
        address from all levels of the cache hierarchy, including both data and instruction caches~\cite{thesisCacheAttacksandRH}. 
        If a cache line at any level of the hierarchy is dirty, meaning 
        that it contains modified data~\cite{docCLFLUSH}, it is written back to memory before being invalidated ~\cite{datasheetIntelReferenceManual, thesisCacheAttacksandRH}.
        The \texttt{clflush} instruction is not privileged and is therefore 
        available in userspace~\cite{datasheetIntelReferenceManual, thesisCacheAttacksandRH}, which makes it particularly suitable 
        ...
        instruction~\cite{docDCCIVAC, datasheetCortexASeries}. Similarly, the \texttt{DC CVAC} instruction cleans a data cache line by address to 
        the Point of Coherency. It is also a 64-bit system instruction and may generate a permission fault 
        when executed at EL0 if such access is enabled, and it may require address translation~\cite{docDCCVAC, datasheetCortexASeries}. 
        The \texttt{DC ZVA} instruction was introduced to efficiently zero out a block of memory~\cite{paperTriggeringRHHardwareFault}. 
        It zeroes a naturally aligned block of N bytes, and is 
        likewise a 64-bit system instruction~\cite{docDCZVA, datasheetCortexASeries}. The ability to preload the data cache with zero 
        values using \texttt{DC ZVA} is new in ARMv8-A. Cache line zeroing behaves similarly to a prefetch by hinting 
        that certain addresses are likely to be used in the future. Nonetheless, zeroing can be significantly 
        faster because it does not require waiting for external memory accesses to complete~\cite{paperTriggeringRHHardwareFault, paperRHonRi3B+}.
    
    \subsubsection{Cache Behavior} \label{subsubsec:cache-behavior}
        \Cref{tab:cache-bypass} presents cache bypass measurements obtained under 
        ...
        cache-bypass mechanisms using \texttt{DC CIVAC} und \texttt{STR}. In addition, POSIX functions measure userspace timings in nanoseconds.
        \begin{table}[ht]
            \centering
            \begin{tabular}{llccccc}
                \hline
                Mode & Unit & RPi4 4~GB & RPi4 8~GB & RPi5 4~GB & RPi5 8~GB & RPi5 16~GB \\
                \hline
            ...
            \hline
            \end{tabular}
            \caption{Variety of cache bypass measurements in user- and kernelspace}
        ...
        while the baseline remains at approximately 43 cycles. This clearly indicates that bypassing the cache substantially increases
        ...
        measurements could not be interpreted due to the Zero Latency Readings Problem described in~\cite{paperPraxisarbeit}, 
        ...
        of \(t = -67.70\). The \textit{Welch t-test} assesses whether the means of two independent samples differ significantly 
        ...
        was calculated as \(d = -3.03\). \textit{Cohen's d} is a standardized measure of effect size that expresses the magnitude 
