\section{Targeted} \label{sec:targeted}
    ...
    \subsection{Reverse Engineering} \label{subsec:reverse-engineering}
        ...
        \subsubsection{DRAMA Approach} \label{subsubsec:drama-approach}
            In the context of reverse engineering DRAM address mappings, it is essential to distinguish 
            ...
            while isolating DRAM from cache effects, using repeated accesses in combination with cache flushes. 
            ...
            \hyperref[fig:combinations-rpi4]{Figure~\ref{fig:combinations-rpi4}} and \hyperref[fig:combinations-rpi5]{Figure~\ref{fig:combinations-rpi5}} 
            depict the frequency of detected combinations for Raspberry Pi 4 and 5 devices, 
            respectively. Occasionally, multiple candidate functions are identified, encompassing rare additional 
            simple bank bits and, in some instances, more complex XOR combinations. These occurrences are 
            noticeably more frequent on the Raspberry Pi 5 devices compared to the Raspberry Pi 4 devices. However, 
            prior studies on DRAM address mapping consistently report that ARM platforms employ only simple 
            XOR functions, typically composed of one or two address bits [73, 79]. Therefore, the 
            infrequently observed complex XOR combinations are assumed to represent false positives, as 
            they lack consistency across runs. Comparisons with DRAM mappings reported in related work 
            demonstrate that the mappings obtained in this study closely resemble those described in [74] and [79].
