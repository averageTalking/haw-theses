\subsection{Verification of Reverse-Engineered DRAM Mapping} \label{subsec:verification}
    A \gls{dram} mapping has been obtained for each test device. Nevertheless, it remains uncertain whether 
    ...
    obtained through direct hardware measurements, as described in~\cite{paperDRAMA}. Nonetheless, this method faces 
    significant practical limitations. Embedded systems often incorporate small chips and 
    multi-layered circuit boards~\cite{thesisDRAMA}, making direct probing difficult. In particular, verification 
    ...
    increases the number of bitflips per second proportionally to the number of sets identified~\cite{paperDRAMA}. 
    ...
    functions~\cite{paperDRAMA, paperVerifingAddressing}. This approach is feasible and provides a practical means of validating the 
    correctness of the reverse-engineered \gls{dram} mappings.

    \subsubsection{Software-based Approach} \label{subsubsec:software-based}
        We conducted the verification of \gls{dram} mapping by using a software-based, latency-driven methodology, 
        following the approach proposed in the DRAMA methodology~\cite{toolDrama}. This method's 
        ...
        reside within the same or different \gls{dram} rows and banks, as shown in~\cref{lst:find-related-addresses}.
        Next, we measure access latencies after flushing the relevant cache lines to ensure that the 
        measurements reflect \gls{dram} behavior rather than cached values. Then we organize the collected latency data  
    ...
    \subsubsection{Rowhammer-based Approach} \label{subsubsec:rowhammer}
        ...
        to an increased rate of bitflips in memory.
        \\
        % Workflow, Problems, and Solutions
        We performed memory allocation \texttt{mmap}, based on a predefined percentage of available system 
        memory, specified by \texttt{MEMORY\_IN\_PERCENT}. Then the \gls{mmu} translated the virtual to physical 
        addresses using pagemap. We grouped all of our physical addresses that correspond to the 
        same \gls{dram} bank into lists according to our reverse-engineered \gls{dram} mapping.
        ...
        iteration, the system checks for resulting bitflips. Once the procedure completes, the total 
        number of detected bitflips is verified and reported.
        \\
        ...
        algorithm shown in~\cref{lst:compute_bank}.
        ...
        intended to induce bitflips. The core idea is to randomly select two distinct addresses within 
        one bank and repeatedly toogle between both in sequence, flushing the cache lines after each access, as illustrated in~\cref{lst:hammer_mechanism}. We repeated this process 
        ...
        \begin{table}[ht]
            ...
            \begin{tabular}{lll}
                ...
                \hline
            \end{tabular}
            \caption{Related Work Mappings}
            \label{tab:mappings}
        \end{table}

        % Reverse-Engineered Mappings
        Multiple attacks were conducted to evaluate both the mappings proposed in related work, as seen in~\cref{tab:mappings}, and the 
        reverse-engineered mappings, with each Raspberry Pi device tested for approximately five hours.
        Across all tests, no bitflips were observed. The resulting mapping is likely correct, as it 
        ...
        The absence of bitflips indicates that either hardware or software mitigations effectively 
