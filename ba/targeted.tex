\section{Address Mapping Reconstruction} \label{sec:targeted}
    ...
    induce bitflips in adjacent victim rows, denoted as "\textit{\hyperref[ch:targeted]{C2. Address Mapping Reconstruction}}". The physical address is 
    composed of bits that are partitioned into rank, channel, bank, row, and column, which 
    collectively determine the location where the data is stored~\cite{paperReliableReverseEngineering}.
    Notably, just like on Intel systems, details of this mapping are not publicly disclosed, unlike the 
    mappings for older CPU models from AMD after they ceased to publish this information with the beginning 
    of microarchitecture 17h~\cite{webZenCPU, webAMDDeveloperCentral, paperREBankAddressing}.
    Knowledge of the \gls{dram} 
    ...
    \gls{dram} layout-aware memory allocators~\cite{paperReliableReverseEngineering}.
    \\
    When the \gls{dram} mapping is known, memory systems become more susceptible to attacks, leading 
    to higher pressure and an increased likelihood of bitflips. The addressing scheme can vary 
    for each system, as it depends on multiple factors such as the processor model, DIMM 
    population on the motherboard, and BIOS settings, according to~\cite{paperReliableReverseEngineering}. Therefore, 
    ...
    described in~\cref{sec:background}. As a consequence of this 
    structure, memory access latencies vary depending on the physical location of data within 
    the hardware~\cite{thesisDRAMA}.
        
    \subsection{Methods for Inferring DRAM Organization} \label{subsec:methods}
        Yun et al.~\cite{paperPALLOC} in 2014, uses a dedicated microbenchmark 
        ...
        \\
        Pessl et al.~\cite{paperDRAMA} in 2016, exploits the row-buffer timing side 
        ...
        \\
        Helm et al.~\cite{paperReliableReverseEngineering} in 2020, extends~\cite{paperDRAMA} by integrating 
        performance counters, which identifies row hits and misses more reliable 
        during the reverse-engineering process.
        \\
        Wang et al.~\cite{paperDRAMDig} in 2020, builds upon~\cite{paperDRAMA} by incorporating 
        detailed knowledge of specific \gls{dram} chip geometries and processor microarchitecture, thereby 
        reducing the search space of the exponential-time~\cite{paperKnockKnock}. For example, \cite{paperDRAMMaUT} applies this method.
        \\
        Xiao et al.~\cite{paperOneBitFlipsOneCouldFlops} in 2016, proposes a graph-based 
        approach to infer physical-to-\gls{dram} mappings. It relies on timing differences caused by 
        row conflicts and, similar to~\cite{paperDRAMA}, primarily aimed at enabling Rowhammer attacks 
        across virtual machines.
        \\
        Common characteristic approaches include the clustering of addresses into 
        sets that experience row conflicts or row hits, indicating placement within the same bank~\cite{paperHalfDouble}. 
        ...
        and tools, including~\cite{paperDrammer, paperHalfDouble, thesisCacheAttacksandRH}.
        \\
        Plin et al.~\cite{paperKnockKnock} 2025, reformulates the identification of 
    ...
    \subsubsection{Knock-Knock Approach} \label{subsec:knock-knock-approach}
        For inferring physical-to-\gls{dram} mappings employs the algebraic framework proposed in~\cite{paperKnockKnock} 
        as described in the Methods section. This approach formulates parity mask identification 
        as a linear algebra problem. By computing the nullspace of conflicting address sets, it directly 
        derives bank and row functions without iterative function validation. We quickly realized that this 
        approach is not applicable to our systems because, as described in~\cite{paperKnockKnock}, a closed page policy prevents 
        us from obtaining any part of the mapping. Given this constraint, we examined a closed-page policy from 
        a theoretical standpoint. Prior work has shown that the memory controller's page policy significantly 
        affects Rowhammer behavior and that one-location hammering remains feasible even under a closed-page 
        policy~\cite{paperAnotherFlip, paperNethammer}. These findings indicate that triggering Rowhammer does not fundamentally depend on 
        an open-page policy or precise knowledge of the physical \gls{dram} address mapping. For the evaluation,
        we focused on testing one-location hammering using the \cite{toolFlipfloyd} tool, which we have ported onto our ARM-based platform. 
        Despite multiple measurement runs exceeding 20 hours in total, we did not observe any bitflips. The 
        most plausible explanations are the presence of \gls{trr} or that the deployed 
        \gls{dram} modules are simply not susceptible to Rowhammer-induced disturbances.

    \subsubsection{Row Hit, Row Conflict Distinction} \label{subsec:row-hit-row-conflict-distinction}
        ...
        analysis.
        \\
        This approach builds upon observations from prior work~\cite{paperDRAMA, toolDrama, thesisDRAMA}. In particular, 
        three primary cases are considered: 
        \\
        \textbf{Bank Miss} (see~\cref{fig:cases}a): the addresses reside in different banks and access 
        times remain low, analogous to a Row Hit but across banks.
        \\
        \textbf{Row Hit} (see~\cref{fig:cases}b): An access to a memory address whose corresponding row 
        is already active in the row buffer. Both of these addresses belong to the same bank and the same 
        row, resulting in low access times.
        \\
        \textbf{Row Conflict} (see~\cref{fig:cases}c): An access targets an 
        address within the same bank while a different row is active in the row buffer, resulting 
        in the need to close the previously opened row and open the new one, producing higher access times.

        \begin{figure}[htbp]
            \centering
            \frame{\includegraphics[width=\linewidth]{img/cases.pdf}}
            \caption{Types of access patterns in DRAM}
            \label{fig:cases}
        \end{figure}
        ...
        \\
        We measured memory access latencies to reliably distinguish Row Hits from Row Conflicts
        while isolating \gls{dram} from cache effects, using repeated accesses in combination with cache flushes. 
        Latency histograms constructed from multiple measurement runs, allow identification 
        of distinct peaks. From these histograms, we determine a threshold to separate Row Hits 
        ...
        \begin{figure}[htbp]
            ...
            \caption{Row hit and -conflict histograms}
            \label{fig:row-hit-conflict}
        \end{figure}

        \Cref{fig:row-hit-conflict} illustrates the resulting 
        histograms across all test devices. On the Raspberry Pi 4, 
        We observe Row Hits in the range of approximately 210-260 CPU cycles, while Row Conflicts 
    ...
    \subsubsection{DRAMA Approach} \label{subsec:drama-approach}
        ...
        In this work, not only the original DRAMA ARM port without Hugepages was used, 
        but also the TRRespass DRAMA ARM port, which supports Hugepages.
        \Cref{fig:combinations-rpi4} and \cref{fig:combinations-rpi5} depict the 
        distribution of detected XOR combinations with the original DRAMA port for the Raspberry Pi 4 
        ...
        more complex XOR combinations. We observe these occurrences more often on 
        Raspberry Pi 5 devices than on Raspberry Pi 4 devices. Prior work on 
        reverse-engineering \gls{dram} address mappings consistently reports that ARM platforms rely 
        exclusively on simple XOR functions, which are typically composed of one or two address bits~\cite{paperDRAMA, paperDRAMMaUT}.
        Consequently, this work assumes that the sporadically observed more complex XOR 
        ...
        \\
        When compared to \gls{dram} address mappings reported in related studies, both of our ported ARM DRAMA 
        mappings derived in this work closely resemble those described in~\cite{paperMemoryAware} and~\cite{paperDRAMMaUT}, as summarized
        in~\cref{tab:mappings}.
        ...
        \begin{table}[ht]
            ...
            \begin{tabular}{lll}
                ...
                \hline
                Raspberry Pi 4B & Memory Aware DOS~\cite{paperMemoryAware} & (11), (12), (13), (14) \\
                Raspberry Pi 4B & DRAM MaUT~\cite{paperDRAMMaUT} & (12), (13), (14), (11 XOR 12) \\
                Raspberry Pi 3B+ & Flipping Bits Like a Pro~\cite{paperProFlipping} & (13 XOR 14), (14), (15) \\
                Exynos 7420 & DRAMA~\cite{paperDRAMA} & (14), (15), (16) \\
                Raspberry Pi's 4 & Original DRAMA ARM Port~\cite{paperPraxisarbeit} & (6), (12), (13), (14) \\
                Raspberry Pi's 5 & Original DRAMA ARM Port~\cite{paperPraxisarbeit} & (6), (12) \\
                Raspberry Pi's 4 and 5 & TRRespass DRAMA ARM Port~\cite{paperPraxisarbeit} & (12), (13), (14) \\
                \hline
            \end{tabular}
            \caption{Mapping comparison with prior work}
            \label{tab:mappings}
        \end{table}

    \subsection{Verification} \label{subsec:verification}
        For each test device, a \gls{dram} address mapping has been derived. Whereas, the completeness and 
        ...
        as reported in prior work~\cite{paperDRAMA}. In general, embedded systems are characterized by small chips and 
        multi-layered circuit boards~\cite{thesisDRAMA}. On Raspberry Pi devices, direct hardware-level verification is 
        ...
        If the inferred functions are correct, their application should increase the bitflip rate 
        proportionally to the number of identified sets~\cite{paperDRAMA}. Nevertheless, this method is often impractical due 
        ...
        same model exhibits Rowhammer bitflips~\cite{paperDrammer,thesisCacheAttacksandRH}.
        \\
        % Software-based Verification
        A purely \textbf{software-based} evaluation constitutes an alternative approach, which verifies timing 
        differences across a larger set of addresses~\cite{paperDRAMA}. This method is feasible and allows validation 
        without hardware access.
        \\
        The verification can be performed using the DRAMA methodology~\cite{paperDRAMA,toolDrama}. In this approach, we measuredmemory 
        access latencies and constructed histograms to distinguish row hits from row 
        conflicts. Address pairs predicted to map to the same \gls{dram} bank and row, denoted as row hits, or to the same 
        bank but different rows, indicates as row conflicts, are identified. After flushing 
        the relevant cache lines to minimize caching effects, we measured access times. Timing histograms are then used to calibrate 
        a threshold separating row hits from row conflicts. The \gls{dram} mapping is validated by comparing the 
        measured latencies with the predicted relationships, as described in the \hyperref[subsec:drama-approach]{previous section}.
        \\
        ...
        several measurements (see~\cref{fig:calibrate}). On Raspberry Pi 4 devices, with the \gls{dram} mapping \texttt{6, 12, 13, 14}, hammerpi 
        ...
        \begin{figure}[htbp]
            \centering
            \frame{\includegraphics[width=0.55\linewidth]{img/calibration.pdf}}
