...
\section{Attack Primitives}
  ...
  to as attack primitives [106, 58]. These primitives define the necessary properties of memory accesses that 
  enable the electrical disturbance of \gls{dram} cells required for inducing bit flips.
  \begin{itemize}
    \item \textbf{P1.} Uncached \label{prim:uncached}
    \item \textbf{P2.} Fast \label{prim:fast}
    \item \textbf{P3.} Targeted \label{prim:targeted}
  \end{itemize}
  
  The first prerequisite, denoted as \textbf{\hyperref[prim:uncached]{P1. Uncached}}, requires that memory accesses bypass 
  ...
  \\
  The second prerequisite, \textbf{\hyperref[prim:fast]{P2. Fast}}, demands that memory accesses occur at a rate that outpaces 
  ...
  \\
  The third prerequisite, \textbf{\hyperref[prim:targeted]{P3. Targeted}}, specifies that memory accesses must be directed to 
