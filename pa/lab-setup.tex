\section{Lab Setup} \label{sec:lab-setup}
  ...
  \subsection{System Setup} \label{subsec:system-setup}
    % Github Setup
    The initial configuration of the laboratory environment involved setting up the necessary repositoriy for development~\cite{toolHamOnRaspi}.
    for coding purposes on all test devices, which contained the experimental programs, ported and adapted code, 
    and the proof-of-concept implementation.
    \\
    % Pi Setup
    For the Raspberry Pi systems, we generated both public and private keys for \textit{WireGuard} and \textit{\gls{ssh}} connections. 
    The corresponding public keys were transmitted to enable secure access on all devices. We tested 
    all hosts and systems to confirm proper operation. We established direct access from the laptop to the Pi systems 
    by placing the authorized keys in the \texttt{.ssh/authorized\_keys} file on each device and added all relevant IP 
    addresses to the \texttt{/etc/hosts} file on the laptop. We also configured \textit{Visual Studio Code} with the 
    ...
    prepared for experimentation. All platforms were based on the ARMv8 architecture, 
    ...
    The term ARM denotes a family of \gls{risc} architectures~\cite{webARMandX86}, within which ARMv7-A and ARMv8-A constitute two major 
    application-profile generations. The ARMv7-A architecture is limited to 32-bit execution, 
    whereas ARMv8-A, supports both 32-bit and 64-bit computation~\cite{paperTriggeringRHHardwareFault}. Although ARMv7-A processors remain in 
    use, ARMv8-A processors have become the predominant architecture in contemporary industrial applications.
    On the Raspberry Pi 4 devices, older system installations were used. These systems ran Arch Linux ARM and reported 
    a \gls{cpu} identified as an ARMv7 Processor revision 3 (v7l) on all Raspberry Pis 4. The kernel versions differed between 
    ...
    requirements consisted of the Raspberry Imager~\cite{webRaspberryImager} installed on the laptop and backups of the Raspberry Pi, including 
    ...
    The kernel was rebuilt natively on the target system to enable the required memory management features 
    while maintaining compatibility with the existing Raspberry Pi environment. The procedure began with a 
    full system update, including updating the package index and upgrading all installed packages, followed 
    by a system reboot to ensure that all updates were applied correctly.
    \\
    Subsequently, all necessary build dependencies were installed. These included standard compilation tools 
    ...
    commit of the \texttt{rpi-6.12.y}.
    \\
    To preserve compatibility with the currently running system, the configuration of the active kernel was 
    reused as the baseline for the new build by copying the existing kernel configuration into the build 
    directory. Based on this configuration, kernel options were adjusted to enable support for the HugeTLB 
    file system and Transparent Hugepages within the memory management subsystem. The correctness of these 
    configuration changes was verified directly in the kernel configuration file.
    \\
    After configuration, the kernel image, kernel modules, and device tree were compiled using all 
    available processor cores. The compiled kernel modules were installed into the system. The newly 
    built kernel image, along with the corresponding device 
    tree binaries and overlays required for system boot, was then deployed to the boot firmware directory. 
    A reboot was performed to activate the new kernel.
    \\
    Finally, the successful installation was verified by checking the running kernel version and confirming 
    the availability of Hugepage support through system interfaces, ensuring that both HugeTLB and Transparent 
    ...
    instructions that are relevant in Section~\hyperref[subsec:flush-cache]{Uncached}, are capable of executing kernel modules
