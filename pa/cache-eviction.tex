%% MAKE BETTER
    ...
    %% Check "Eviction Set Structure" in "Eviction Development"
    %% Check "Eviction Function" in "Eviction Development"
    %% Check "Cache Set Determination" in "Eviction Development"
    %% Check Parameter of "Eviction Strategies" in "Eviction Development"
...
\subsection{Cache eviction} \label{subsec:cache-eviction}
    ...
    memory accesses [~Grand Pwning Unit]. This approach exploits the relatively simple cache structures 
    of integrated \gls{gpu}s, which can be evicted efficiently in order to achieve main-memory accesses [81, 155].

    \textbf{CPU cache eviction} is considerably more complex [TODO]. This complexity arises from higher cache associativity, 
    ...
    timing and detailed knowledge of cache set mappings are available [72, 108].
    ...
    \subsubsection{Cache Organization} \label{subsubsec:cache-organization}
        ...
        consequence, multiple distinct memory addresses can also map to the same cache location [72].
        \\
        An illustrative example is shown in Figure~\ref{fig:direct-mapped-cache}, which depicts a cache with a total size of 4~KB, 
        ...
        \begin{figure}[htbp]
            ...
            \caption{Direct-mapped cache [72]}
        ...
        cache is referred to as fully associative [72]. Modern processors, however, typically employ 
        set-associative caches, which organize cache lines into multiple sets [72]. In an N-way set-associative 
        ...
        congruent addresses, as illustrated in Figure~\ref{fig:n-way-associative-cache}. 
        When all cache lines of a set are occupied and a new block needs to be loaded, the cache replacement 
        policy determines which existing cache line is evicted in order to accommodate the new data [72].

        \begin{figure}[htbp]
            \centering
            \frame{\includegraphics[width=\linewidth]{img/n-way-associative-cache.pdf}}
            \caption{N-way associative cache [72]}
        ...
        \end{table}
        Tables \ref{tab:rpi4-cache} and \ref{tab:rpi5-cache} summarize the cache configurations of the Raspberry 
        Pi 4 and Raspberry Pi 5, respectively, as obtained using the lscpu --cache command. These tables provide 
        ...
        \noindent
        \gls{cpu} caches can be indexed and tagged using either virtual or physical addresses [72]. When virtual 
        addresses are used, it is possible for the same physical address to be cached in multiple cache lines, 
        which can negatively affect performance [72]. Cache lines are uniquely identified by a tag, which may 
        be derived from either the virtual or the physical address [72].
        \\
        ...
        translation via the \textbf{\gls{tlb}} is required before cache access.
        \\
        ...
        address for tagging. This approach offers lower latency than \textbf{\gls{pipt}} caches because index lookup can proceed 
        in parallel with \textbf{\gls{tlb}} translation, although tag comparison must still wait for the physical address.
        \\
        On the Raspberry Pi 4, the L1 instruction cache, L1 data cache, and L2 cache behave as \gls{pipt} caches [172, 123]. 
        On the Raspberry Pi 5, the L1 instruction and L1 data caches are \gls{vipt}, but they behave effectively as \gls{pipt}
        4-way set-associative caches [122].
    ...
    \subsubsection{Cache Management} \label{subsubsec:cache-management}
        ...
        policy, which aims to select the entry that is least likely to be used in the near future [Quote of 72]. The chosen 
        ...
        the longest time [72]. In contrast, a pseudo-random replacement policy selects a cache entry to evict based 
        on a pseudorandom number generator [72].
        \\
        On the Raspberry Pi 4, the L1 instruction and L1 data caches use an \gls{lru} replacement policy, while the L2 
        cache employs a pseudo-\gls{lru} replacement policy [172, 123]. On the Raspberry Pi 5, the L1 instruction and 
        ...
        the lower-level cache are also stored in the higher-level cache [72]. Caches are considered exclusive if 
        a cache line can reside in only one of two cache levels [72]. If a cache is neither inclusive nor exclusive, 
        it is referred to as non-inclusive [72].
        \\
        ...
        to the L1 instruction cache [122]. The L3 cache is neither strictly inclusive nor strictly exclusive, but 
        rather non-inclusive, with a dynamic allocation policy that depends on the access pattern [173, 174]. 
        ...
        L2 caches, exhibiting inclusive behavior [173, 174].
    ...
    \subsubsection{Eviction Development} \label{subsubsec:eviction-development}
        ...
        The eviction set structure, as defined in Listing~\ref{lst:eviction-set-structure}, consists of multiple virtual 
        ...
        addresses contained in an eviction set (see Listing~\ref{lst:eviction-function}). During execution, 
        ...
        of the eviction routine is governed by several configuration parameters, namely \texttt{ES\_EVICTION\_COUNTER}, 
        \texttt{ES\_NUMBER\_OF\_ACCESSES\_IN\_LOOP}, and \texttt{ES\_DIFFERENT\_ADDRESSES\_IN\_LOOP}. Together, these 
        ...
        address (see Listing~\ref{lst:cache-set-determination}). This is achieved by computing the set index from the 
        ...
        The libflush library was built for the ARMv8 architecture using \texttt{make ARCH=armv8 CC=gcc}, producing 
        both static and shared libraries. A system-wide installation without Android dependencies was performed using 
        \texttt{sudo make install ARCH=armv8 WITH\_ANDROID=0 CC=gcc}. The provided example application was subsequently 
        built and executed with \texttt{make run ARCH=armv8 WITH\_ANDROID=0 USE\_EVICTION=1 TIME\_SOURCE=monotonic\_clock CC=gcc}, 
        ...
        \texttt{cache-bypass} was modified to simplify the use of \texttt{make}, \texttt{make install}, and 
        \texttt{make uninstall} by forwarding these targets to \texttt{armageddon/libflush}. The \texttt{libflush/example} 
        ...
        sufficiently fast, as previously proposed eviction strategies have been shown to be too slow [179] [72]. 
        In this work, the cache eviction implementation proposed by Lipp et~al. [125] is used. This approach is 
        applicable only to devices with a known eviction strategy [159], and therefore the default configuration 
        provided by the libflush tool is adopted (see Figure~\ref{lst:default}).
