\subsection{Flush cache} \label{subsec:flush-cache}
    ...
    \subsubsection{Instruction Types} \label{subsubsec:instruction-types}
        ...
        \texttt{CLFLUSH} instruction to flush the data associated with a given address from the cache hierarchy~\cite{paperFlippingBits}.
        ...
        naturally aligned block of N bytes~\cite{docDCZID}. As described in~\cite{paperRHonRi3B+}, “The ability to preload the data cache 
        with zero values using the \texttt{DC ZVA} instruction is new in ARMv8-A. Cache line zeroing behaves in a similar fashion 
        ...
        accesses to complete~\cite{paperTriggeringRHHardwareFault}."
        \\
    ...
    \subsubsection{Kernel Module} \label{subsubsec:kernel-module}
        ...
        that the exclusion of memory barrier instructions, such as \gls{dsb} and \gls{isb}, leads to a higher effectiveness in triggering bit flips. 
        ...
        including its memory system, at both EL1 and EL0~\cite{docSCTLREL1}. The \texttt{\gls{uci}} field is located at bit position \texttt{26}~\cite{docSCTLREL1, docSystemControlRegister} 
