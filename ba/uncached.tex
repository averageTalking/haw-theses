\section{Direct DRAM Activation} \label{sec:uncached}
    In the following, each attack primitive is discussed in turn, beginning with "\textit{\hyperref[ch:uncached]{C1. Direct DRAM Activation}}", which 
    establishes the foundation for inducing repeated \gls{dram} row activations. This challenge 
    ...
    \\
    We consider several cache-bypass techniques to enable the memory access behavior required for triggering 
    Rowhammer~\cite{presRHAttacksWalkthrough, toolRowhammering}. These techniques include the use of non-temporal memory 
    ...
    \gls{dram} locations~\cite{paperTriggeringRHHardwareFault}. \gls{cpu} caches significantly obstruct this process by intercepting repeated accesses, 
    ...
    via the \texttt{clflush} instruction, non-temporal accesses, and eviction-based techniques - \cite{paperTriggeringRHHardwareFault} demonstrates 
    ...
    data from the cache or by preventing data from being cached in the first place~\cite{toolRPI3}.
    ...
    \subsection{Non-temporal Accesses} \label{subsec:non-temporal-accesses}
        ...
        memory instructions~\cite{paperANewApproach}. \gls{nt} accesses are intended for data with low temporal locality, i.e., 
        ...
        been shown to significantly facilitate Rowhammer attacks on x86 systems~\cite{paperANewApproach}. In contrast, on ARMv8 architecture
        \gls{nt} load and store instructions (e.g., \texttt{LDNP} and \texttt{STNP}) merely act as advisory hints to the memory 
        system rather than enforcing uncached behavior~\cite{paperDrammer}. Empirical results indicate that accessed data 
        may still be cached in practice~\cite{paperTriggeringRHHardwareFault}. Moreover, multiple consecutive writes to the same address are 
        collected within a single write-combining buffer, with only the last write being committed to \gls{dram}~\cite{toolRowhammering}.
    ...
    \subsection{Uncached Memory} \label{subsec:uncached-memory}
        ...
        cache~\cite{paperDrammer}. This method is particularly suitable for ARM-based mobile architectures, such as ARMv7 and 
        ARMv8, where cache flush instructions may require elevated privileges and non-temporal stores may still 
        be subject to caching in practice~\cite{toolRowhammering}. A key advantage of this technique is that it operates entirely 
        ...
        bypass \gls{cpu} caches, thereby enabling Rowhammer triggering from user space~\cite{paperDrammer, paperTriggeringRHHardwareFault}. Even so, subsequent 
        defenses, such as~\cite{paperGuardION}, have rendered Rowhammer bugs triggered in this way non-exploitable. 
