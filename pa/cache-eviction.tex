\subsection{Cache eviction} \label{subsec:cache-eviction}
    ...
    \\
    In the context of \gls{gpu} cache eviction, prior work has shown that by cleverly evicting the cache of an 
    integrated \gls{gpu}, access to main memory can be achieved with significantly reduced latency, enabling 
    memory accesses~\cite{paperPwning,paperTriggeringRHHardwareFault}. This approach exploits the relatively simple cache structures 
    of integrated \gls{gpu}s, which can be evicted efficiently in order to achieve main-memory accesses~\cite{paperOpenGL, thesisDRAMA, paperPwning}.
    \\
    CPU cache eviction is considerably more complex~\cite{paperARMageddon}. This complexity arises from higher cache associativity, 
    ...
    \\
    This section first outlines cache management, 
    ...
    \subsubsection{Cache Characteristics} \label{subsubsec:cache-characteristics}
        \cref{tab:rpi4-cache} and \cref{tab:rpi5-cache} summarize the cache configurations of the Raspberry 
        ...
        \begin{table}[h!]
            \centering
            \caption{RPi4 Cache using '\texttt{lscpu {-}{-}cache}' command}
        ...
        \begin{table}[h!]
            \centering
            \caption{RPi5 Cache using '\texttt{lscpu {-}{-}cache}' command}
    ...
    \subsubsection{Cache Management} \label{subsubsec:cache-management}
        ...
        cache entries~\cite{paperARMageddon}. The heuristic used to decide which cache entry to evict is referred to as the cache replacement 
        policy, which aims to select the entry that is least likely to be used in the near future~\cite{paperTriggeringRHHardwareFault}. The chosen 
        ...
        a cache line can reside in only one of two cache levels~\cite{thesisExploitingMicroOpt, thesisCacheAttacksandRH}. If a cache is neither inclusive nor exclusive, 
        it is referred to as non-inclusive.
        \\
        On the Raspberry Pi 4, the L2 cache is inclusive with respect to the L1 cache~\cite{docCortexA72}. On the Raspberry Pi 5, the 
    ...
    \subsubsection{Eviction Development} \label{subsubsec:eviction-development}
        In this work we use the cache eviction mechanism based on the libflush source of the armageddon tool~\cite{toolArmageddon} proposed by Lipp et al.~\cite{paperARMageddon} and relies on the 
        construction and use of eviction sets. 
        \\
        % Eviction Set Structure
        The eviction set structure, as defined in~\cref{lst:eviction-set-structure}, consists of multiple virtual 
        ...
        addresses contained in an eviction set (see~\cref{lst:eviction-function}). During execution, 
        ...
        address (see~\cref{lst:cache-set-determination}). This is achieved by computing the set index from the 
        ...
        Libflush~\cite{toolArmageddon} was built for the ARMv8 architecture.
        ...
        \\
        % Eviction Strategy
        Cache attack techniques such as \textit{Flush+Reload} and \textit{Flush+Flush} typically rely on the 
        unprivileged x86 \texttt{CLFLUSH} instruction to evict cache lines. In contrast, ARMv8-A \gls{cpu}s do 
        not provide an unprivileged flush instruction~\cite{paperDrammer}, which makes cache eviction strategies necessary instead~\cite{thesisCacheAttacksandRH}. 
        ...
        The Lipp et al.~\cite{paperARMageddon,toolArmageddon} approach is 
        applicable only to devices with a known eviction strategy~\cite{thesisDRAMA}, and therefore the default configuration 
        provided by the libflush tool~\cite{toolArmageddon} is adopted (see~\cref{lst:default}).
        \input{listings/default.h.tex}

        The relevant configuration parameters are defined in the default libflush~\cite{toolArmageddon} configuration. The parameter 
        ...
        accessed per iteration in order to reliably evict a target cache line. \\
        The parameter 
        \texttt{ES\_NUMBER\_OF\_ACCESSES\_IN\_LOOP} specifies how often the eviction loop is repeated to ensure eviction 
        under noisy conditions. \\
        Finally, \texttt{ES\_DIFFERENT\_ADDRESSES\_IN\_LOOP} defines the number of distinct 
